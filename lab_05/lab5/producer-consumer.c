...
struct sembuf producer_P[] = {{BUFFER_EMPTY, -1, 0},{BIN_SEM, -1, 0}};
struct sembuf producer_V[] ={{BIN_SEM, 1, 0},{BUFFER_FULL, 1, 0}};
struct sembuf consumer_P[] ={{BUFFER_FULL, -1, 0},{BIN_SEM, -1, 0}};
struct sembuf consumer_V[] ={{BIN_SEM, 1, 0},{BUFFER_EMPTY, 1, 0}};

void Producer(int nomer, pid_t pid, int sem_id, char* buf, int* pos, int* character)
{
    while (1)
    {
        ...
        printf("\x1b[7mPROD %d with pid = %d", nomer, pid);
        printf("\x1b[7mproduce %c to buffer[%d]\n", buf[*pos], *pos );

        // проверка выхода из буфера
        if (*pos == BUF_SIZE - 1)
            *pos = 0;
        else
            *pos += 1;
...
void Consumer(int nomer, pid_t pid, int sem_id, char* buf, int* pos)
{
    while (1)
    {
        ...
        // взять из буфера
        printf("\x1b[0mConsumer #%d <- ", nomer);
        printf("\x1b[0mtake buffer[%d] = %c\n", *pos, buf[*pos]);
...
int main()
{
    ...
    *character = 0;
    ...
    for (int i = 0; i < COUNT_PRODUCERS; i++)
    {
        ...
        if (!pid)
            Producer(i + 1, getpid(), sem_id, mem_ptr, producer_pos, character);
        else
            count_processes++;
    }
    ...
    for (int i = 0; i < COUNT_CONSUMERS; i++)
    {
        ...
        if (!pid)
            Consumer(i + 1, getpid(), sem_id, mem_ptr, consumer_pos);
        else
            count_processes++;
    }
